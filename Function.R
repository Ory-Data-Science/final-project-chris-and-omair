...
List <- List%>%
  ...
  mutate(OPIATE=replace(OPIATE,str_detect(NONPROPRIETARYNAME,
                                          'OXYCODONE|MORPHINE|FENTAN|HYDROCODONE|MORPHONE|CODEINE|TAPENTADOL|CODEINE|BELLADONNA|BUTORPHANOL|TRAMADOL'),'YES'))%>%
  mutate(DEASCHEDULE=replace(DEASCHEDULE,str_detect(NONPROPRIETARYNAME, 'OXYCODONE|MORPHINE|FENTAN|HYDROCODONE|MORPHONE|TAPENTADOL'),'CII'))
...
refine <- function(pattern) {
  ...
  for (f in filenames) {
    ...
    assign(paste('OpiateRXs', name[5], sep = ''), State, envir = .GlobalEnv)
    
    nationalname <- paste('NationalData', name[5], '.csv', sep = '')
    statename <- paste('OpiateRXs', name[5], '.csv', sep = '')
    
    setwd('~/GitHub/final-project-chris-and-omair')
    
    write_csv(National, nationalname)
    write_csv(State, statename)
  }
}